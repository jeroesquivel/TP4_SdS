...
\newcommand{\videolinkNcien}{}        % N = 100,  k = 10^3
\newcommand{\videolinkNmil}{}         % N = 1000, k = 10^3
\newcommand{\videolinkKcien}{}        % k = 10^2
\newcommand{\videolinkKdiezmil}{}     % k = 10^4
...
\begin{frame}{Sistema real y modelo matemático}
    \begin{columns}[T]
        \begin{column}{0.56\textwidth}
            \textbf{Sistema real}
            \begin{itemize}\setlength\itemsep{2pt}
                \item Partículas en un recinto cerrado que rebotan
                      contra un obstáculo fijo central.
                \item A diferencia del esquema event-driven (TP3, choques
                      instantáneos), se modela el \emph{contacto} con una
                      fuerza elástica blanda.
            \end{itemize}

            \vspace{0.2cm}
            \textbf{Ecuaciones del modelo}
            \begin{itemize}\setlength\itemsep{2pt}
                \item Movimiento de Newton:
                      $m_i\,\dot{\vect{v}}_i = \sum_j \vect{F}_{ij}$.
                \item Fuerza de contacto a pares:
                      \[
                          \vect{F}_{ij} = -k\,\xi\,\hat{\vect{e}}_{ij},
                          \quad
                          \xi = r_i + r_j - |\vect{x}_i - \vect{x}_j|,
                      \]
                      activa sólo si $\xi > 0$.
                \item Energía total $E_{\text{tot}} = E_{\text{kin}} + E_{\text{pot}}$
                      debe conservarse (validación del $\Delta t$).
            \end{itemize}
        \end{column}
        \begin{column}{0.44\textwidth}
            ...
            \begin{tikzpicture}[scale=1.0]
                ...
                \fill[violet!70!black] (0,0) circle (0.22);
                \foreach \x/\y in {1.0/0.4, -1.2/0.6, 0.2/-1.5, 1.5/-1.0, -0.8/-1.7, -1.6/-0.3}
                \fill[green!60!black] (\x,\y) circle (0.10);
                \foreach \x/\y in {-0.4/1.3, 1.8/0.2}
                \fill[violet!70!black] (\x,\y) circle (0.10);
            \end{tikzpicture}\\[6pt]
            {\footnotesize Fuerza activa cuando dos partículas (o partícula--obstáculo, partícula--borde) se solapan.}
        \end{column}
    \end{columns}
\end{frame}
